...
public class BalconyOtherType_2 extends Balcony{


    public BalconyOtherType_2(int w, int h, int index,int S,int MinCount, int MaxCount, int ConcavityRate, PApplet app) {
    ...
    public void CBOneTemplate(int i, int j, GRBLinExpr expr , GRBVar[][][] status ) {             //每个模板的判定区域
        expr.addTerm(1.0, status[i][j][2]);
        expr.addTerm(1.0, status[i - 1][j][2]);
        expr.addTerm(1.0, status[i][j + 1][2]);
    }

    public void DefineBetween(int i, int j, ArrayList<Balcony> AllBalcony, GRBLinExpr expr , GRBVar[][][] status){
    ...
    public void drawThisBalcony_2(int i, int j, int col ,int size){
        app.fill(col);
